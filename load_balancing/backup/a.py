...
def Int2(name,veh_num):
    ...
    for i in range(veh_num):
        temp1 = []
        for j in range (veh_num):
            temp1.append(m.addVar(lb = -1, ub = 1,vtype=GRB.INTEGER,name=name + "_"  + str(i)+ "_"  + str(j)))
        temp.append(temp1)
    return temp
...
def Cont2(name,veh_num):
    ...
    for i in range(veh_num):
        temp1 = []
        for j in range (veh_num):
            temp1.append(m.addVar(vtype=GRB.CONTINUOUS,name= name + "_"  + str(i) + "_"  + str(j)))
        temp.append(temp1)
    return temp
...
try:
    ...
    for i in range (veh_num):
        for j in range(veh_num):
            ...
            m.addConstr(t1[i][j] == and_([t[i][j],c[i][j]]))
            m.addConstr((t1[i][j] == 1) >> (e[i] - e[j] >= 1))
    ...
    for i in range(veh_num):
        for j in range (veh_num):
            ...
            m.addConstr((b[i][j] == 1) >> (b2[i][j] == 0)) #oi = oj
            m.addConstr((b[i][j] == 0) >> (b2[i][j] - 1 >= 0)) #oi != oj
            ...
            m.addConstr((c[i][j] == 1) >> (c2[i][j] == 0)) #li = lj
            m.addConstr((c[i][j] == 0) >> (c2[i][j] - 1 >= 0)) #li != lj
            m.addConstr((c_prime[i][j] == 0) >> (c2[i][j] == 0))
            m.addConstr((c_prime[i][j] == 1) >> (c2[i][j] - 1 >= 0))
    ...
    for i in range(veh_num):
        for j in range (veh_num):
            ...
            m.addConstr((a[i][j] == 1) >> (e[i] - e[j] >= W[i][j]))
    ...
    #         m.addConstr((p3[i][j] == 1) >> (p5[i][j] == 0))
    #         m.addConstr((p3[i][j] == 0) >> (p5[i][j] - 1 >= 0))
    #         m.addConstr(p6[i][j] == or_([p3[i][j],c[i][j]]))
    #         m.addConstr(p7[i][j] == and_([p6[i][j],p[i]]))

    #         m.addConstr(d_prime[i] - d_prime[j] >= M * (f[i][j] - 1))
    #         m.addConstr(d_prime[i] - d_prime[j] <= M * (f[i][j]))
    #         # f_ij = 1 => d'_i > d'_j
    #         # m.addConstr(d_prime[i] - d_prime[j] >= 1 - f[i][j])
    #         # # f_ij = 0 => d'_i <= d'_j
    #         # m.addConstr(d_prime[i] - d_prime[j] <= -f[i][j])
    #         m.addConstr(p8[i][j] == and_([p7[i][j],f[i][j]]))
    #         m.addConstr(p9[i][j] == and_([p7[i][j],f[j][i]]))
    #         m.addConstr((p8[i][j] == 1) >> (d_prime[i] - d_prime[j] >= T))
    #         m.addConstr((p9[i][j] == 1) >> (d_prime[j] - d_prime[j] >= T))
    ...
    # for i in range(veh_num):
    #     for j in range (veh_num):
    #         m.addConstr((a[i][j] == 1) >> (o[i] == o[j] and l[i] == l[j] and e[i] < e[j]),"same_lane")
    #         m.addConstr((a[i][j] == 0) >> (o[i] != o[j] or l[i] != l[j] or e[i] >= e[j]),"same_lane1")
    #         m.addConstr(b[i][j] == e[j] * a[i][j],"same_lane2")
    ...
    for v in m.getVars():
        if 'd_' in v.varName or 'e_' in v.varName or 'l_' in v.varName or 'o_' in v.varName:
            print('%s %g' % (v.varName, v.x))

...
for j in range(veh_num % N):
    eachlane[j] += 1
print(eachlane,np.var(eachlane))
...
try:
    ...
    o1 = Binary2("o1",veh_num,N)
    a1 = Int2(- N - 1,N-1,"a1",veh_num,N)
    a2 = Int2(0,N-1,"a2",veh_num,N)
    c = Int1(0,M-1,"c",veh_num)
    l = Int1(0,M-1,"l",veh_num)
    l1 = Binary2("l1",veh_num,M)
    c1 = Int1(-N - 1,N-1,"c1",veh_num)
    a3 = Int2(- M - 1,M-1,"a3",veh_num,M)
    a4 = Int2(0,M-1,"a4",veh_num,M)
    b = Binary3("b",veh_num,M,N)
    ...
    for i in range(veh_num):
        m.addConstr(o[i] >= 0)
        m.addConstr(o[i] <= N - 1)
        m.addConstr(l[i] - allVeh[i].lane <= allVeh[i].k)
    ...
    for i in range (veh_num):
        allVeh[i].outLane = leave[i]
        allVeh[i].leaveLane = out[i]

...
class Vehicle:
    def __init__(self,ID,arrival_time,inlane,outlane,leavelane,number):
        ...
        for i in range (self.decision_num + 1,0,-1):
            self.lane[N - i] = (inlane)
            if self.lane[N - i] < self.outLane:
                inlane += 1
            elif self.lane[N - i] > self.outLane:
                inlane -= 1
        self.lane.reverse()
    ...
    for i in range (veh_num):
        ...
        if allVeh[i].passingOrder < allVeh[veh].passingOrder and allVeh[i].lane[n] == allVeh[veh].lane[n]:
            if allVeh[i].lane[n + 1] == allVeh[veh].lane[n + 1] and allVeh[i].lane[n] == allVeh[i].lane[n+1]:
                ...
                time.append(allVeh[i].decision_depart[n] + allVeh[i].w_equal[veh] + 1)
...
def myFunc2(e):
    return e.lane[0],e.intersection_arrival

            
def enter_passing(allVeh,veh_num,N):
    passes1 = 0
    passes2 = veh_num
    for i in range(veh_num):
        if allVeh[i].decision_depart[0] == 0:
            allVeh[i].intersection_arrival = allVeh[i].arrival_time
        else:
            allVeh[i].intersection_arrival = allVeh[i].decision_depart[0] + 15
    allVeh.sort(key=myFunc2)
    for i in range(N):
        leaves = []
        for j in range (veh_num):
            if allVeh[j].outLane == i:
                leaves.append(allVeh[j].leaveLane)
        leaves.sort(key=myFunc1)
        # print(leaves)
        leave_count = 0
        for j in range(veh_num):
            if allVeh[j].outLane == i:
                allVeh[j].leaveLane = leaves[leave_count]
                if leaves[0] == leaves[leave_count]:
                    allVeh[j].passingOrder = passes1
                    passes1 += 1
                else:
                    allVeh[j].passingOrder = passes2
                    passes2 += 1
                leave_count += 1
            
    return allVeh
    # for i in range(veh_num):
    #     print(allVeh[i].ID,allVeh[i].outLane,allVeh[i].leaveLane,allVeh[i].arrival_time,allVeh[i].passingOrder)
...
def schedule_enter(allVeh,N):
    ...
    allVeh = enter_passing(allVeh,veh_num,N)
    for i in range(veh_num):
    ...
    for i in range(veh_num):
        # print(allVeh[i].ID,allVeh[i].intersection_arrival,allVeh[i].outLane,allVeh[i].leaveLane,allVeh[i].passingOrder,allVeh[i].scheduled_enter)
...
# A = [Vehicle("A_1",16,0,0,1,1),Vehicle("A_2",17,0,1,0,2)]

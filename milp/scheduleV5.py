...
def myFunc1(e):
    ...
    return e.curr_expected_arrival + 0.5 * e.child_diff
...
def assign_passing(allVeh,veh_num,n):
    for i in range (veh_num):
        ...
        allVeh[i].curr_lane = allVeh[i].lane[n + 1]
    for i in range (veh_num):
        child = []
        for j in range(i + 1,veh_num):
            if allVeh[i].curr_lane == allVeh[j].curr_lane:
                child.append(allVeh[j].curr_expected_arrival - allVeh[i].curr_expected_arrival)
        if len(child) > 0:
            allVeh[i].child_diff = min(child)
    allVeh.sort(key=myFunc1)
    for i in range(veh_num):
        allVeh[i].passingOrder = i
    
        # print(n,allVeh[i].ID,allVeh[i].child_diff,allVeh[i].expected_arrival,allVeh[i].lane,allVeh[i].passingOrder)
    return allVeh
...
def findmaxpred(veh,allVeh,veh_num,n,N):
    ...
    for i in range (veh_num):
        ...
        if allVeh[i].passingOrder < allVeh[veh].passingOrder and allVeh[i].lane[n] == allVeh[veh].lane[n]:
            if allVeh[i].lane[n + 1] == allVeh[veh].lane[n + 1]:
                time.append(allVeh[i].decision_depart[n] + 1)
            else:
                time.append(allVeh[i].decision_depart[n] + 2)
    maxtime = max(time)
...
def deal_enter(veh,allVeh,veh_num):
    time = [0]
    for i in range (veh_num):
        if allVeh[i].passingOrder < allVeh[veh].passingOrder:
            if allVeh[i].outLane == allVeh[veh].outLane:
                time.append(allVeh[i].scheduled_enter + allVeh[veh].w_equal)
            elif allVeh[i].leaveLane == allVeh[veh].leaveLane:
                time.append(allVeh[i].scheduled_enter + allVeh[veh].w_plus)
